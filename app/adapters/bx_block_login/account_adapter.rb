  ...
  class AccountAdapter
    ...
    def login_account(account_params)
      ...
      when 'sms_account'
        ...
        #   full_phone_number: phone, activated: true
        # )
        account = AccountBlock::Account.find_by("(full_phone_number = ? or phone_number = ?) AND activated = ?",phone, phone, true)
      when 'email_account'
        email = account_params.email.downcase
        # account = AccountBlock::EmailAccount.where('LOWER(email) = ?', email).where(activated: true).first
        account = AccountBlock::Account.where('LOWER(email) = ?', email).where(activated: true).first
